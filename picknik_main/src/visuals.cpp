  ...
  : nh_("~")
{
  ...
  visual_tools_.reset(new mvt::MoveItVisualTools(robot_model->getModelFrame(),
                                                 "/picknik_main/markers", planning_scene_monitor));
  ...
  visual_tools_display_.reset(new mvt::MoveItVisualTools(
      robot_model->getModelFrame(), "/picknik_main/highres_markers", planning_scene_monitor));
  // visual_tools_display_->deleteAllMarkers(); // clear all old markers

  // ------------------------------------------------------------------------------------------------------
  // Load RobotState VisualTools for Start State
  start_state_.reset(new mvt::MoveItVisualTools(
      robot_model->getModelFrame(), "/picknik_main/grasping_markers", planning_scene_monitor));
  ...
  goal_state_.reset(new mvt::MoveItVisualTools(
      robot_model->getModelFrame(), "/picknik_main/trajectory_markers", planning_scene_monitor));
  ...
  // product_perception_.reset(new mvt::MoveItVisualTools(robot_model->getModelFrame(),
  // "/picknik_main/product_perception",
...
bool Visuals::isEnabled(const std::string& setting_name)
{
  ...
  ROS_ERROR_STREAM_NAMED("visuals", "isEnabled() key '"
                                        << setting_name
